...
with open('eng_rus_countries.json') as j:
    map = json.loads(j.read())
    eng_names = dict((v,k) for k, v in map.iteritems())
...
output_f.write('<attributes class="edge" mode="static">\n' +
               ...
               '<attribute id="2" title="decade" type="string"/>\n</attributes>\n<nodes>')
cities_id = {}
countries_id = {}
edges_id = {}
ci_id = 0
...
with open('file_base.json', 'r') as f:
    ...
    for piece in data:
        ...
        decade = data[piece]['decade']
        d.append([countries_together, author, century, decade])
...
for country in set(countries):
    if country not in countries_id:
        ...
        co_id += 1
    try:
        s = '<node id="%s" label="%s">\n<attvalues/>\n</node>' % (eng_names[country], eng_names[country]) # str(countries_id[country]))
        output_f.write(s.encode('utf-8'))
    except KeyError:
        continue
output_f.write('</nodes>\n<edges>')
...
for piece in d:
    ...
    for pair in itertools.combinations(piece[0],2):
        ...
        #print pair[0], pair[1]
        try:
            s = '<edge id="%s" source="%s" target="%s">' % (str(ed_id), eng_names[pair[0]], eng_names[pair[1]]) #str(countries_id[pair[0]]), str(countries_id[pair[1]]))
